...
We first apply the TF-IDF feature extraction to convert job description body text to a vector.
...
Filter out job postings with empty descriptions.

```{python}
df0 = df.filter(col("BODY").isNotNull() & (trim(col("BODY")) != ""))
```

Break down the job description body text into a list of words.
...
Remove meaningless words e.g. pronouns and prepositions.
...
Compute TF and IDF to assemble feature vectors.
...
To understand the skill requirements, we pick out the top 30 most frequent words in job descriptions.
...
We cluster the job postings into 7 groups using the K-Means algorithms.
...
For each job postings cluster, we visualize the most frequent key words using a word cloud.
